...
function* CategoriesWorker() {
    try {
        const data: ICategoryMiniBlock[] = yield call(requestCategories)
...
function* subValueWorker({payload: {type, page, limit}}: TaskAction) {
    try {
        const data: IProduct[] = yield call(requestSubValues, type, page, limit)

        yield put(setSubValues(data))
    } catch (error) {
        const _error = error as string
        yield put(setError(_error))
    }
}
